    ...
    return (
      <div className={"postcard " + (p.isTop ? "postcard-top" : "")}>
        ...
        <div className={"postcard-media " + (url ? "has-media" : "")}>
          ...
          {isEd && (<div className="postcard-media-input"><input className="media-input" type="text" placeholder="Paste image, video, IG, or FB URL..." value={mediaInput} onChange={e => setMediaInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter") handleMediaSave(p.id); if (e.key === "Escape") { setEditingMedia(null); setMediaInput(""); } }} autoFocus /><div className="media-btn-row"><button className="media-btn secondary" onClick={() => { setEditingMedia(null); setMediaInput(""); }}>Cancel</button><button className="media-btn primary" onClick={() => handleMediaSave(p.id)}>Save</button></div></div>)}
          {url && !isEd && (
            <div className="postcard-media-filled">
              ...
              <div className="postcard-media-actions"><button onClick={() => { setEditingMedia(p.id); setMediaInput(url); }}>{"\u270E"}</button><button onClick={() => handleMediaRemove(p.id)}>{"\u2715"}</button></div>
            </div>
          )}
        </div>
        ...
        <div className="postcard-secondary">{[
          { val: p.likes, label: p.platform === "Facebook" ? "Reactions" : "Likes" },
          { val: p.shares, label: "Shares" },
          { val: p.comments, label: "Comments" },
          { val: p.platform === "Facebook" ? (p.clicks || 0) : (p.saves || 0), label: p.platform === "Facebook" ? "Clicks" : "Saves" },
        ].map(m => (<div key={m.label} className={"postcard-sec-item " + (m.val === 0 ? "zero" : "")}><span className="postcard-sec-val">{m.val}</span><span className="postcard-sec-label">{m.label}</span></div>))}</div>
      </div>
    );
  }
